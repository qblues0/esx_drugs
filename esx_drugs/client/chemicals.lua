...
function chemicalsmenucheck()
	...
	ESX.TriggerServerCallback('ddx_drugs:CheckLisense', function(cb)
		if cb then
			wasOpen = true
			OpenChemicalsMenu()
		else
...
function OpenChemicalsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'ChemicalsConvertionMenu', {
		...
		menu.close()
		menuOpen = false
		TriggerServerEvent('ddx_drugs:ChemicalsConvertionMenu', data.current.name, 1)
			...
			if IsControlJustReleased(0, Keys['E']) and not isPickingUp then
				...
				ESX.TriggerServerCallback('ddx_drugs:canPickUp', function(canPickUp)
					...
					if canPickUp then
						...
						TriggerServerEvent('ddx_drugs:pickedUpChemicals')
					else
