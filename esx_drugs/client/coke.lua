		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.CokeProcessing.coords, true) < 5 then
			...
			if IsControlJustReleased(0, Keys['E']) and not isProcessing then
				...
				if Config.LicenseEnable then
					ESX.TriggerServerCallback('ddx_license:checkLicense', function(hasProcessingLicense)
...
function ProcessCoke()
	...
	TriggerServerEvent('ddx_drugs:processCocaLeaf')
	...
	while timeLeft > 0 do
		...
		if GetDistanceBetweenCoords(GetEntityCoords(playerPed), Config.CircleZones.CokeProcessing.coords, false) > 5 then
			...
			TriggerServerEvent('ddx_drugs:cancelProcessing')
			break
		end
	end
	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if IsControlJustReleased(0, Keys['E']) and not isPickingUp then
				...
				ESX.TriggerServerCallback('ddx_drugs:canPickUp', function(canPickUp)
					...
					if canPickUp then
						...
						TriggerServerEvent('ddx_drugs:pickedUpCocaLeaf')
					else
